<template>
    <section class="showChose chooseAddress-com">
      <section class="address">
        <section class="title addressTitle">
          <h4>配送至</h4>
          <span @click="closeAdd()">×</span>
        </section>
        <section class="title selectTitle">
          <div class="area" @click="provinceSelected()">{{Province?Province:info[province-1].name}}</div>
          <div class="area area1" @click="citySelected()" :class="City?'':'active'">{{City?City:'请选择'}}</div>
          <div class="area area2" @click="districtSelected()" :class="District?'':'active'" v-show="City">{{District?District:'请选择'}}</div>
        </section>
        <ul>
...
export default {
  ...
  methods: {
    ...
    getProvinceId: function(code,input,index) {
      ...
      this.areaInfo.province=this.info[index].name
    //   console.log(this.areaInfo.province)
    },
    ...
    getCityId: function(code, input, index) {
      ...
      this.areaInfo.city=this.showCityList[index].name
    //   console.log(this.areaInfo.city)
    },
    ...
    getDistrictId: function(code, input, index) {
      ...
      this.areaInfo.district=this.showDistrictList[index].name
    //   console.log(this.areaInfo.district)
    //   this.showChose = false;
      this.$emit('getUsrArea',this.areaInfo)
      this.$emit('showSelectArea')
    },
